...
function HomeMusics({ filter }: { filter: string }) {
  const [data, setData] = useState<{
    musics: { playlist: string; musicList: string[] }[];
  } | null>(null);
  ...
  function listMusic() {
    ...
    if (data !== null) {
      rows.push(
        <div
          className="sm:text-3xl text-sm flex m-3 border-solid border-2
                  grid grid-cols-3 gap-1"
        >
          <h1 className="m-2 col-span-2">
            <strong>Titre</strong>
          </h1>
          <h1 className="m-2 col-span-1">
